...
func InitMySQLWithGorm(mysqlConfig *setting.MysqlConfig) (*gorm.DB, error) {
	dsn := mysqlConfig.Dsn()
	...
	sqlDB.SetConnMaxIdleTime(mysqlConfig.FreeMaxLifetime * time.Minute)

	return db, nil
}

func CloseMySQLWithGorm(db *gorm.DB) error {
	if db == nil {
		return nil
	}
